...
# Get argument first
nnBlobPath = str((Path(__file__).parent / Path('vehicle-detection-adas-0002.blob')).resolve().absolute())
if len(sys.argv) > 1:
...
colorCam.setPreviewSize(672, 384)
        ...
        while True:
            ...
            if frame is not None:
                for detection in detections:
                    ...
                    Zdepth = int(detection.spatialCoordinates.z)
                    Depth2 = int(detection.spatialCoordinates.z)
                    cv2.putText(frame, f"X: {int(detection.spatialCoordinates.x)} mm", (x1 + 10, y1 + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color)
                    cv2.putText(frame, f"Y: {int(detection.spatialCoordinates.y)} mm", (x1 + 10, y1 + 45), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color)
                    cv2.putText(frame, f"Z: {round((detection.spatialCoordinates.z * 0.001), 3)} m", (x1 + 10, y1 + 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color)
                    

                    cv2.rectangle(frame, (x1, y1), (x2, y2), (0,0,255), 2)
                    cv2.rectangle(frame, (x1, (y1 - 28)), ((x1 + 78), y1), (0,0,255), cv2.FILLED)
                    cv2.putText(frame, str(label), (x1 + 5, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255))
                    cv2.putText(frame, "{:.2f}".format(detection.confidence*100), (x1 + 50, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255))

                        #Zrect = int(50000/detection.spatialCoordinates.z)

                    if 2800 > Zdepth > 100:
                        ZDepth = 255
                    elif 3700 > Zdepth > 2801:
                        ZDepth = 170
                    elif 5000 > Zdepth > 3701:
                        ZDepth = 120
                    elif 7000 > Zdepth > 5001:
                        Zdepth = 80
                    else:
                        Zdepth = 40
                        
                    Xframe = int(detection.spatialCoordinates.x)
                    #cv2.rectangle(depthFrameColor, (0, 40), (100, 60), (255, 255, 255), 3)

                    #cv2.rectangle(depthFrameColor, (0, 40), (Zrect, 60), (255, 255, 255), cv2.FILLED)
